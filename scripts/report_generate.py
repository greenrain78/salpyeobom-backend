...
async def fetch_rows() -> list[dict]:
    await Tortoise.init(db_url=settings.DATABASE_URL, modules={"models": ["app.models.adl_raw"]})
    rows = (
        await AdlRawRecord.filter(id__gte=185, id__lte=214).order_by("lifeog_date", "id").values()
    )
...
def analyze(rows: list[dict]) -> dict:
    ...
    weeks = [rows[0:7], rows[7:14], rows[14:21], rows[21:30]]
    wk = [
        {
            "label": f"{i}주차",
    ...
    return {
        "rows": rows, "dates": dates, "aix": aix, "night": night,
        "bath": [r["bath_count_d"] for r in rows], "out": [r["outgoing_count_d"] for r in rows],
        "weeks": wk, "w1_aix": wk[0]["aix"], "w4_aix": wk[3]["aix"],
        "w1_night": wk[0]["night"], "w4_night": wk[3]["night"],
        "w1_bath": wk[0]["bath"], "w4_bath": wk[3]["bath"],
        "night_max": night[ni], "night_max_date": dates[ni],
        "aix_min": aix[ai], "aix_min_date": dates[ai],
        "profile": rows[0], "event_date": rows[-1]["emergency_date"],
    }
...
def chart_activity(an: dict) -> Path:
    ...
    ax2 = ax1.twinx()
    ax2.plot(xs, an["night"], color="#B42318", lw=1.6, marker="s", ms=3, alpha=0.9, label="야간 이상비")
    ax2.set_ylabel("야간 이상비", fontsize=8.5)
    _despine(ax1, ("left", "bottom"))
    _despine(ax2, ("right",))
    ax1.axvline(len(xs) - 1, color="#B0B6BF", ls=(0, (3, 3)), lw=1)
    ax1.annotate("사건일 · 낙상", xy=(len(xs) - 1, 0), xytext=(len(xs) - 1, max(an["aix"]) * 0.5),
                 fontsize=8, color="#B42318", ha="right", va="center")
    ...
    ax1.set_title("활동량 붕괴와 야간 이상비 급등", loc="left", fontsize=10.5,
                  fontweight="bold", color="#22272E", pad=10)
    fig.tight_layout()
...
def chart_selfcare(an: dict) -> Path:
    ...
    ax1.grid(axis="y", color="#EDF0F3", lw=0.8)
    ax1.set_axisbelow(True)
    ax1.set_title("자기관리(목욕) 위축과 외출 유지", loc="left", fontsize=10.5,
                  fontweight="bold", color="#22272E", pad=10)
    fig.tight_layout()
...
def three_line(t, *, zebra=False):
    ...
    b = OxmlElement("w:tblBorders")
    specs = {
        "top": (H_ACCENT, "12"), "bottom": (H_ACCENT, "12"),
...
def data_table(doc, header, rows, *, aligns=None, zebra=False, widths=None):
    ...
    for i, htext in enumerate(header):
        cell_text(t.rows[0].cells[i], htext, bold=True, size=8.5, color=ACCENT,
                  align=(aligns or {}).get(i, WD_ALIGN_PARAGRAPH.LEFT), before=2, after=2)
        cell_margins(t.rows[0].cells[i])
    for row in rows:
        cells = t.add_row().cells
        for i, v in enumerate(row):
            cell_text(cells[i], str(v), size=9, align=(aligns or {}).get(i, WD_ALIGN_PARAGRAPH.LEFT),
                      before=2, after=2)
...
# 5. 보고서
# ─────────────────────────────────────────────────────────────────────────────
def build(an, c1, c2) -> Path:
    prof = an["profile"]
    sex = "여성" if prof["sex"] == "F" else "남성"
    doc = Document()
    set_base_font(doc)
    doc.core_properties.title = "고령자 위험 예측 보고서 — 대상자 661"
    ...
    run(eyebrow, "SALPYEOBOM   RISK PREDICTION REPORT", size=9.5, bold=True, color=ACCENT, tracking=60)
    t1 = para(doc, after=2)
    ...
    meta = kv_block(
        doc,
        [
            ("문서 번호", f"SPB-RR-661-{REPORT_DATE.strftime('%Y%m%d')}"),
    ...
    para(doc, before=30)
    disc = para(doc)
    run(disc, "본 보고서는 AI 예측 모델의 출력을 LLM이 자동 서술한 문서입니다. "
              "위험 점수는 모델 산출 가정값이며, 임상적 진단이나 의료적 판단을 대체하지 않습니다.",
        size=8.5, color=MUTE, italic=True)
    ...
    h1(doc, "00", "핵심 요약")
    risk_banner(doc, "고위험 HIGH", "RISK LEVEL", 0.87,
                "낙상·외상에 의한 응급 이송", "1주 이내 발생 가능성")
    para(doc, before=10)
    kpi_cards(doc, [
        ("활동량 aix_d", f"{an['w4_aix']:.0f}", f"{pct(an['w1_aix'], an['w4_aix'])}  vs 1주", RISK),
        ("야간 이상비", f"{an['w4_night']/1000:.1f}k", f"{pct(an['w1_night'], an['w4_night'])}  vs 1주", RISK),
        ("목욕 횟수", f"{an['w4_bath']:.0f}", f"{pct(an['w1_bath'], an['w4_bath'])}  vs 1주", RISK),
    ])
    para(doc, before=8)
    p = para(doc)
    run(p, f"대상자 {prof['care_recipient_id']}({prof['age']}세·{sex}·독거)의 최근 30일 원격 ADL "
           "데이터를 분석한 결과, 예측 모델은 본 대상자를 ")
    run(p, "고위험(4단계)", bold=True, color=RISK)
    run(p, "으로 분류하였다. 핵심 근거는 ①주간 활동량의 지속적 붕괴, ②자기관리(목욕) 활동의 위축, "
    ...
    kv_block(doc, [
        ...
        ("거주 형태", f"독거 · {prof['house_structure']} · 방 {prof['room_no']}칸 · 욕실 {prof['bath_location']} · {prof['district']}"),
    ...
    run(p, "독거(발견·구조 지연), 고령(낙상 시 중상 가능성), 옥내 욕실·계단 동선 등 물리적 낙상 위험이 기저에 존재한다.")

    h1(doc, "02", "분석 방법 및 데이터")
    p = para(doc)
    run(p, "원격 ADL 센서가 산출하는 일별 지표(활동량·수면·목욕·외출·야간 이상비)를 30일치 수집하여, "
           "예측 모델이 사고·사망 위험을 0–1 점수와 4단계 등급으로 산출한다. 본 LLM 보고서는 모델 출력을 "
           "재예측하지 않고, 모델이 근거로 삼은 지표 추세를 사람이 이해할 수 있도록 서술·시각화한다.")
    p = para(doc)
    run(p, "데이터 품질 주의 — night_aix_ratio 등 일부 비율 지표는 원본 계산 편차가 커 절대값보다 ‘추세’로 "
           "해석한다. 외출 시계열의 254/255 코드는 센서 무신호 표식으로 분석에서 제외했다.",
        size=8.5, color=MUTE)

    h1(doc, "03", "위험 판정 근거 — 30일 추세")
    p = para(doc)
    run(p, f"활동량(aix_d)은 초기 평균 {an['w1_aix']:.0f}에서 3주차 17까지 급락한 뒤 회복하지 못했고 "
    ...
    data_table(
        ...
        [[w["label"], w["range"], f"{w['aix']:.0f}", f"{w['night']:,.0f}", f"{w['bath']:.1f}", f"{w['out']:.1f}", f"{w['sleep']:.0f}"]
         for w in an["weeks"]],
    ...
    kv_block(doc, [
        ...
        ("현장 · 이송 · 치료", f"{prof.get('on_site','—')} · {prof.get('hospital_transfer','—')} · {prof.get('hospital_treatment','—')}"),
        ("예측–실제 일치", "일치 — 고위험 판정 구간 내 낙상·응급 이송 발생"),
    ])
    p = para(doc, before=6)
    run(p, "30일 누적 패턴(주간 쇠약 + 야간 불안정 + 외출 유지)이 실제 실외 계단 낙상으로 귀결되어, 모델의 "
